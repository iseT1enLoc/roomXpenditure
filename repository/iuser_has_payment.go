...
type UserHashPaymentRepository interface {
	...
	GetExpensesFilteredFromStartDateToEndDate(ctx context.Context, userID uuid.UUID, roomID uuid.UUID, start_date, end_date *time.Time, page, limit int) ([]models.UserPaymentResponse, int64, error)
	GetExpensesFilteredFromStartDateToEndDateOfOneMember(ctx context.Context, userID uuid.UUID, roomID uuid.UUID, start_date, end_date *time.Time, page, limit int) ([]models.UserPaymentResponse, int64, error)
}
